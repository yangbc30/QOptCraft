...
def MatLogCompV(N1=False,N2=False,txt=False,exp=False):
	...
	for i in range(N1,N2+1):
		...
		if exp==2:
			...
			comparison_file.write(f"N = {i}: expm(logm_3(A)) = expm(logm_4(A))? {comp_3_y_4}, expm(logm_3(A)) = expm(logm_5(A))? {comp_3_y_5}, expm(logm_4(A)) = expm(logm_5(A))? {comp_4_y_5}\n")

		else:

			comparison_file.write(f"N = {i}: logm_3(A) = logm_4(A)? {comp_3_y_4}, logm_3(A) = logm_5(A)? {comp_3_y_5}, logm_4(A) = logm_5(A)? {comp_4_y_5}\n")
