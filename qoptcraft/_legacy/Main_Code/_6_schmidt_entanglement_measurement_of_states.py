...
def StateSchmidt(file_input_state=True,file_input_matrix=True,state_input=False,U_input=False,
	file_output=True,filename_state=False,filename_matrix=False,base_input=False,vec_base=[[False,False],[False,False]],
	acc_d=3,txt=False,fidelity=0.95):
	'''
	...
	file_input_state, filename_state, _, acc_d=input_control(module=6,file_input=file_input_state,M_input=U_input,
		file_output=file_output,filename=filename_state,txt=txt,acc_d=acc_d)
	file_input_matrix, filename_matrix, _, acc_d=input_control(module=6,file_input=file_input_matrix,M_input=U_input,
	...
	for i in range(len(state_basis_vectors)):
		...
		post_entanglement_leading=schmidt_rank_vector(output_state_leading,vec_base_leading,list(modes_per_partition))
		...
		if txt==True:
			...
			print(f"Our array's exact basis elements and their probabilities after passing the circuit are located in the iteration {i} of '{filename_state}_leading.txt'.")
			...
			print(f"The fidelity={fidelity} array's exact basis elements and their probabilities after passing the circuit are located in the iteration {i} of '{filename_state}_reduction_{fidelity}.txt'.")
