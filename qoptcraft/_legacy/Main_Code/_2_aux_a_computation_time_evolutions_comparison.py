...
def StoUEvolComp(file_output=True,m1=False,m2=False,n1=False,n2=False,txt=False,tries=1,vec_base=[[False,False],[False,False]],inverse=False,comparison_matrix='haar'):
	"""
	...
	if txt==True:
		...
		print("In this code, computation times of the three implemented methods in the main algorithm 2 are compared.\n\n")
	...
	if file_output==True:
		...
		workbook = xlsxwriter.Workbook(f'computation_comparisons_m_{m1}to{m2}_n_{n1}to{n2}_{comparison_matrix}.xlsx') # create the workbook

		cell_format=workbook.add_format({'center_across' : True, 'border' : True, 'align': 'center', 'valign': 'vcenter'})
		#cell_format.set_border() (another way to assign classes)
		cell_format_number=workbook.add_format({'center_across' : True, 'left' : True, 'right' : True, 'align': 'center', 'valign': 'vcenter'})
		cell_format_number_end=workbook.add_format({'center_across' : True, 'left' : True, 'right' : True, 'bottom' : True, 'align': 'center', 'valign': 'vcenter'})
	...
	for cont in range(tries):
		...
		for k in range(m1,m2+1):
			...
			while comparison_matrix is not 'haar' and comparison_matrix is not'qft':
				...
				comparison_matrix==input("Available options selected via writing what is between parenthesis for each:\n1) Haar random matrices (haar)\n2) Quantum Fourier Transform matrices (qft)")
			...
			for j in range(n1,n2+1):
				...
				if file_output==True:
					...
					scores = {"m" : f"{k}", "n" : f"{j}", "M" : f"{comb_evol(j,k)}", "Method 1" : t_inc_1/(10.0**9), "Method 2" : t_inc_2/(10.0**9), "Method 2b" : t_inc_2b/(10.0**9), "Method 3" :t_inc_3/(10.0**9)} # your score data
					i = 0          
					# line index
					for name in scores:
						...
						if k==m2 and j==n2:
							...
							worksheet.write(col, i, scores[name], cell_format_number) # write score at row i and column col
